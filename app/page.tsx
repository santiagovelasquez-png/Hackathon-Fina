    ...
    <div className="min-h-screen bg-[#0F172A] text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0F172A]/80 backdrop-blur-md border-b border-white/10">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/login"
              className="text-sm text-slate-300 hover:text-white transition-colors px-4 py-2 cursor-pointer">
              Iniciar sesión
            </Link>
            <Link href="/login"
      ...
      <section className="animated-gradient-bg pt-32 pb-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative">
          {/* Badge */}
          <div className="animate-fade-in inline-flex items-center gap-2 bg-white/10 border border-white/20 backdrop-blur-sm rounded-full px-4 py-1.5 text-xs font-medium text-blue-200 mb-8">
            <Zap size={12} fill="currentColor" />
            Powered by Gemini 2.5 Pro · Hackathon 2025
          </div>

          <h1 className="animate-fade-up delay-100 text-5xl md:text-6xl font-bold tracking-tight leading-tight mb-6">
            Encuentra el talento correcto,
            <br />
            <span className="landing-gradient-text">más rápido con IA</span>
          </h1>

          <p className="animate-fade-up delay-200 text-lg text-blue-100/80 max-w-2xl mx-auto mb-10 leading-relaxed">
      ...
      <section className="bg-[#0F172A] border-y border-white/10 py-12 px-6">
        <div className="max-w-4xl mx-auto grid grid-cols-3 gap-8 text-center">
          {[
            ...
            <div key={i} className={`animate-fade-up delay-${(i + 1) * 100 + 200}`}>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              },
              {
                ...
                desc: "Invita a los mejores con un clic. La IA selecciona las 6 preguntas más relevantes para el cargo y el candidato responde a su ritmo.",
                detail: ["Sin agendar reuniones", "Respuestas asíncronas", "Análisis IA incluido"],
              },
            ].map(({ step, Icon, title, desc, detail }, i) => (
              <div key={i}
                className={`animate-fade-up delay-${(i + 1) * 200} relative bg-[#1E293B] rounded-2xl p-7 border border-white/10 hover:border-blue-500/40 transition-all group`}>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              },
              {
                ...
                title: "Entrevistas por Voz IA",
                desc: "Candidatos responden en video/audio. La IA analiza respuestas y genera un reporte de competencias detallado.",
              },
            ].map(({ Icon, color, glow, title, desc }, i) => (
              <div key={i}
                className={`animate-fade-up delay-${(i + 1) * 200} bg-[#1E293B] rounded-2xl p-7 border border-white/10 hover:border-blue-500/30 transition-all`}>
                <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${color} flex items-center justify-center mb-5 shadow-lg ${glow}`}>
                  <Icon size={20} className="text-white" />
                </div>
                <h3 className="font-bold text-white text-lg mb-3">{title}</h3>
      ...
      <section className="bg-[#111827] py-24 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            {/* Mock ranking card */}
            <div className="animate-float bg-[#1E293B] rounded-2xl p-6 border border-white/10 shadow-2xl shadow-black/40">
      ...
      <section className="animated-gradient-bg py-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center relative">
          ...
          <Link href="/login"
            className="inline-flex items-center gap-3 bg-white text-slate-900 font-bold px-10 py-4 rounded-2xl hover:bg-blue-50 transition-all shadow-2xl shadow-black/30 cursor-pointer text-base animate-pulse-glow">
            <Users size={20} />
            Crear cuenta gratis
          </Link>
      ...
      <footer className="bg-[#0F172A] border-t border-white/10 px-6 py-8">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-slate-500 text-center">
            Construido para Fina + Platanus Hackathon 2025 · Powered by Gemini 2.5 Pro
          </p>
